...
Download All 默认保留链上地址（原地址审计版）。可选“新包构建版”只迁移
主包模块路径，保留地址常量、字符串及外部依赖身份，并把未迁移源码放在
`audit/sources/`。这不是找回部署前原工程：原测试、编译配置和锁文件不一定
能从链上恢复。新包初始化和既有对象兼容性需单独检查。

两个版本均包含 `verify.mjs`。解压后执行 `node verify.mjs` 运行构建、测试并
